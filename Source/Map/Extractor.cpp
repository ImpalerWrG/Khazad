...
//FIXME: the ugly hack
int16_t Extractor::PickMaterial(int16_t TileType, int16_t basematerial, int16_t veinmaterial, DFHack::t_matglossPair constructionmaterial, DFHack::t_occupancy occupancy)
{
    static uint16_t Unknown = DATA->getLabelIndex("MATERIAL_UNINITIALIZED");
    static uint16_t LayerStone = DATA->getLabelIndex("MATERIALCLASS_LAYER_STONE");
    static uint16_t Soil = DATA->getLabelIndex("MATERIALCLASS_SOIL");
    static uint16_t VeinStone = DATA->getLabelIndex("MATERIALCLASS_VEIN_STONE");

    //return Unknown;

    int16_t TileMaterial = TileMaterialPicker[TileType];
    if (TileMaterial != -1 && TileMaterial < DATA->getNumMaterials())
    {
        return TileMaterial;
    }

    int16_t TileMaterialClass = TileMaterialClassPicker[TileType];
    int16_t DefaultMaterial = -1;
    ...
    int16_t ConstructionMaterial = ResolveMatGlossPair(constructionmaterial);
...
int16_t Extractor::ResolveMatGlossPair(t_matglossPair MatPair)
{
    static uint16_t Unknown = DATA->getLabelIndex("MATERIAL_UNINITIALIZED");

    if (MatPair.type != -1 && MatPair.index != -1)
    {
        int16_t PotentialMaterial = -1;
        ...
        for (int16_t MaterialClass = 0; MaterialClass < DATA->getNumMaterialClasses(); MaterialClass++)
        {
