...
public:
    ...
    const char* getStaticMaterialName(char* Buffer, int16_t MaterialTypeID, int16_t TextureID);
    const char* getStaticTextureName(char* Buffer, int16_t MaterialTypeID, int16_t TextureID);

    Ogre::Image* ConvertToOgreImage(int16_t ImageID);
    Ogre::Image* makeImage(int16_t MaterialTypeID, int16_t TextureID);

    Ogre::MaterialPtr getOgreMaterial(int16_t MaterialTypeID, int16_t SurfaceTypeID);

    Ogre::MaterialPtr makeStaticMaterial(Ogre::Image* SourceImage, const char* MaterialName);
