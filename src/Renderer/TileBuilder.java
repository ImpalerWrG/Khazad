...
public class TileBuilder implements Serializable {
	...
	ConcurrentHashMap<FaceShape, RawMeshData> MesheDataMap;	// map tileshape to mesh for retrival

	public TileBuilder() {
		MesheDataMap = new ConcurrentHashMap<FaceShape, RawMeshData>();
	}

	public RawMeshData getMeshData(FaceShape Shape) {
		RawMeshData TargetMeshData = MesheDataMap.get(Shape);
		...
		if (TargetMeshData == null) {
			if (Shape.getFaceDirection() == Direction.DIRECTION_NONE) {
				...
				return new RawMeshData(TargetMeshData);
			} else {
	...
	public Mesh getMesh(FaceShape Shape, TextureAtlasCoordinates AtlasCoords, FaceCoordinate FaceCoords) {
		RawMeshData NewData = getMeshData(Shape);
		if (NewData != null) {
			NewData.convertTextureCoordinates(AtlasCoords);
			NewData.translateVertexs(FaceCoords);
			return NewData.createMesh();
	...
	private RawMeshData createFlatFaceMesh(FaceShape Shape) {
		...
		if (Triangle1 || Triangle2) {
			return new RawMeshData(Vertices, Normals, TextureCoords, Indexes);
		} else {
			return null;
		}
	}

	private RawMeshData createSideFaceMesh(FaceShape Shape) {
		...
		float YRight = 0;
		int LeftCorner = 0;
		int LeftAdjacentCorner = 0;
		int RightCorner = 0;
		int RightAdjacentCorner = 0;
		Vector3f Normal = new Vector3f();

		switch (Shape.getFaceDirection()) {
			case DIRECTION_SOUTH:
				XLeft = -MapCoordinate.HALFCUBE;
				YLeft = -MapCoordinate.HALFCUBE;
				XRight = MapCoordinate.HALFCUBE;
				YRight = -MapCoordinate.HALFCUBE;
				LeftCorner = Cube.getSouthWestCorner();
				RightCorner = Cube.getSouthEastCorner();
				LeftAdjacentCorner = Adjacent.getNorthWestCorner();
				RightAdjacentCorner = Adjacent.getNorthEastCorner();
			...
			case DIRECTION_NORTH:
				...
				XRight = -MapCoordinate.HALFCUBE;
				YRight = MapCoordinate.HALFCUBE;
				LeftCorner = Cube.getNorthEastCorner();
				RightCorner = Cube.getNorthWestCorner();
				LeftAdjacentCorner = Adjacent.getSouthEastCorner();
				RightAdjacentCorner = Adjacent.getSouthWestCorner();
			...
			case DIRECTION_WEST:
				...
				XRight = -MapCoordinate.HALFCUBE;
				YRight = -MapCoordinate.HALFCUBE;
				LeftCorner = Cube.getNorthWestCorner();
				RightCorner = Cube.getSouthWestCorner();
				LeftAdjacentCorner = Adjacent.getNorthEastCorner();
				RightAdjacentCorner = Adjacent.getSouthEastCorner();
			...
			case DIRECTION_EAST:
				...
				XRight = MapCoordinate.HALFCUBE;
				YRight = MapCoordinate.HALFCUBE;
				LeftCorner = Cube.getSouthEastCorner();
				RightCorner = Cube.getNorthEastCorner();
				LeftAdjacentCorner = Adjacent.getSouthWestCorner();
				RightAdjacentCorner = Adjacent.getNorthWestCorner();
	...
	private RawMeshData createSlopeFaceMesh(FaceShape Shape) {
